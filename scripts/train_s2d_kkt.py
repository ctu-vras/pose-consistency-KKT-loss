...
if __name__ == '__main__':
    ...
    net_s2d.load_state_dict(torch.load("../weights/network_weights_s2d", map_location=device))
    net_s2d.to(device)
    ...
    copyfile('train_s2d2_kkt.py', output_file + '/train_script_s2d_kkt.py')
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(trainloader):
            ...
            if True:
                ...
                for b in range(0, rpz.shape[0]):
                    ###### Find patch with steepest pitch
                    idx1, idx2 = np.where(~np.isnan(rpz[b, 2, :, :]))
                    K = np.argmax(
                        np.abs(rpz[b, 1, idx1, idx2] - output_rpz.cpu().detach().numpy()[b, 1, idx1, idx2].ravel()))
                    r1 = idx1[K] - 3 + 3
                    r2 = idx1[K] + 4 + 3
                    c1 = idx2[K] - 4 + 4
                    c2 = idx2[K] + 5 + 4
                    indexes_minidem = [r1, r2, c1, c2]
                    RPZ = rpz[b, :, idx1[K], idx2[K]]
                    YAW = yaw[b, idx1[K], idx2[K]]
                    x_in = torch.tensor([RPZ[0], RPZ[1], YAW, 0, 0, RPZ[2]])
                    rb_optimizer.optimize_network(net_s2d, input_w_mask[b].to(device), x_in, points, indexes_minidem,
                                                  label_dem[b, :, :, :], weights[b, :, :, :])
